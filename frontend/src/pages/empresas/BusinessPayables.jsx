                            ...
                            <div className="md:col-span-5 flex gap-3">
                                <div className="flex-1 grid grid-cols-2 gap-3 bg-white dark:bg-slate-900 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                                    <div className="space-y-1">
                                        ...
                                        <Input 
                                            type="date" className="h-8 text-xs bg-slate-50"
                                            value={pay.extension_date}
                                            disabled={pay.status === 'pago'}
                                ...
                                <div className="flex flex-col gap-1 justify-center">
                                    <Button 
                                        ...
                                        disabled={pay.status === 'pago'}
                                        className="h-8 w-8 text-slate-400 hover:text-cyan-600 hover:bg-slate-100 disabled:opacity-20" 
                                        onClick={() => handleEditFull(pay)}
                                    >
                                        <Edit size={16}/>
                                    </Button>
                                    <Button 
                                        variant="ghost" size="icon" 
                                        disabled={pay.status === 'pago'}
                                        className="h-8 w-8 text-slate-400 hover:text-red-600 hover:bg-red-50 disabled:opacity-20" 
                                        onClick={() => handleDelete(pay.id, pay.status)}
                                    >
                                        <Trash2 size={16}/>
                                    </Button>
                                </div>
